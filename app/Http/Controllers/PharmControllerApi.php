...
class PharmControllerApi extends Controller
{   
    ...
    public function associateAddPharm(Request $request, $associate_username)
    {   
        // return $request;
        ...
        if($associate_data){
            ...
            if ($user->save())
            {
                ...
                $response = ['success'=>true, 'data'=>"Pharmacy added successfully"];        
            }
            else
    ...
    public function listPharms(Request $request, $associate_username)
    {   
        $validator = Validator::make($request->all(), [  
            'secret_key'                => 'required|string|max:255',
        ]);
        ...
        if($associate_data){
            ...
            $pharmsList = DB::table('pharmacies')
                ->select('username','name','email','zip_code','telephone','pharm_country','pharm_district_province_state','pharm_address')
    ...
    public function account(Request $request, $associate_username,$pharm_username)
    {   
        $validator = Validator::make($request->all(), [  
            'secret_key'                => 'required|string|max:255',
        ]);
        ...
        if($associate_data){
            $associate_id = $associate_data->id;
            $pharm = Pharmacies::where([['username', $pharm_username], ['associate', $associate_id]])->get()->first();
            if($pharm){
                $pharm_id = $pharm->id;

                $order_list = Pharmorder::where('pharm_id', '=', $pharm_id)->get()->all();
                
                $order_list = DB::table('pharmorders')
                ->select('billing_amount_currency','billing_amount_value','billing_cart_total','billing_handling_fee','billing_email_address','billing_name',  
                'billing_orderID','billing_create_time','pharm_username','pharm_name','patient_username','patient_first_name','patient_last_name','patient_middle_name','status')
                ...
                $response = ['success'=>true, 'data'=>"invalid pharmacy username"];
                return response()->json($response, 201);
            }
        }else{
            $response = ['success'=>false, 'data'=>'Incorrect credentials'];
