...
{   
    ...
    {
        // This is use to get the token for Associates model. it uses the same for user.
        \Config::set('jwt.user', Associates::class);
        \Config::set('auth.providers', ['users' => [
                'driver' => 'eloquent',
                'model' => Associates::class,
            ]]);
    }
    ...
    {   
        ...
        $associate_data = \App\Associates::where('email', $email)->get()->first();
        if ($associate_data && \Hash::check($password, $associate_data->password)) // The passwords match...
        {   
            // This gets the user role name and passed into a variable
            $role_name = $associate_data->roles->pluck('name');
            $role_name = $role_name[0];

            $token = self::getToken($email, $password);
            $associate_data->auth_token = $token;
            $associate_data->save();
    ...
    {   
        ...
        $user = new \App\Associates($payload);
        if ($user->save())
        {
            ...
            $user = \App\Associates::where('email', $email)->get()->first();
            ...
            $emailDetails = [
                ...
                'url' => 'https://dashboard.cammedics.com/#/login_associate'
            ];
