...
use Mail;
use App\Mail\ReplyAppointmentMessage;
...
class GeneralAppointmentController extends Controller
{
    ...
    {
        //
        $appointments = Generalappointment::paginate(10);
        // return $result;
    ...
    {
        $data = Generalappointment::where('id', '=', $id)->first();
        $status    = $data->status;
        if($status == 1){ 
            $data->status = 2;
            $data->save();
    ...
    {   
        ...
        $appointment_data = Generalappointment::where('id', '=', $id)->first();
        $appointment_data->reply_subject  = $reply_subject;
        $appointment_data->reply_message  = $reply_message;
        ...
        Mail::to($sender_email)->send(new ReplyAppointmentMessage($emailDetails));
        // ////////////////////////////////////////////////////////////////////////////////////////
