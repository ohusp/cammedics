...
class AssociateDoctorAccount extends Component {
  constructor(props) {
    super(props);
    // ////////////////MODAL    
  ...
  getAccount(doc_id)
  {
    ...
    axios.get(`/api/associate/account/`+doc_id+`?token=${this.state.token}`)
    .then(response => {
      // console.log("ROI Cartoon");
      // console.log(response);
      return response;
  ...
  handlePageChangeGetAccount(pageNumber) {
    // console.log(`active page is ${pageNumber}`);
    // this.setState({activePage: pageNumber});
    axios.get(`/api/associate/account/`+this.state.doc_id+`?token=${this.state.token}&page=`+pageNumber)
    .then(response => {
      return response;
    })
    .then(json => {
...
export default AssociateDoctorAccount;
