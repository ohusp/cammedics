...
class BookAppointment extends Component {
  ...
  componentDidMount()
  {
    ...
    if( this.state.login_as != "admin_user"){
      ...
      axios.get(`/api/admin/list_appointments?token=${this.state.token}`)
      .then(response => {
  ...
  handlePageChange(pageNumber) {
    ...
    axios.get(`/api/admin/list_appointments?token=${this.state.token}&page=`+pageNumber)
    .then(response => {
      return response;
    })
    .then(json => {
  ...
  onSubmit(e)
  {
    ...
    if(
      ...
      axios.post(`/api/admin/reply/appointment_message/`+this.state.id+`?token=${this.state.token}`, appointment_data)
      .then(response => {
        return response;
      })
      .then(json => {
  ...
  changeStatus(id){
    axios.get(`/api/admin/general_appointments/change_status/`+id+`?token=${this.state.token}`)
    .then(response => {
      console.log("ROI Cartoon");
      console.log(response);
      return response;
  ...
  render() {
    ...
    return (
      ...
      <div className="animated fadeIn"> 
        <Row> 
          <Col xs="12" sm="12">
            <h3>General Appointment Requests</h3>  
          </Col>
        </Row><br></br> 
        <Row>
            ...
            <Col xs="12" lg="12">
              <Card>
                ...
                {/* // ///////////////// ////////////// */}
                <CardHeader>
                  <i className="fa fa-align-justify"></i> List of General Appointments Requests  
                </CardHeader>
                <CardBody>
                    ...
                    <tbody>
                      {
                        ...
                        // ////////////////////////////////////////////////////////////
                        this.state.list.map(appointment=>{
                          if(appointment.status == 1){
                            this.state.status = <Button size="sm" onClick={() => this.changeStatus(appointment.id)} active color="danger" title="Make as read" aria-pressed="true"><i className="fa fa-close"></i></Button>;
                          }else{
                            this.state.status = <Button size="sm" onClick={() => this.changeStatus(appointment.id)} active color="success" title="Make as unread" aria-pressed="true"><i className="fa fa-check"></i></Button>;
                          }
                          ...
                          return(
                            <tr key={appointment.id}>
                              ...
                              <td>
                                <Button size="sm" onClick={() => this.toggleView(appointment.name, appointment.email, appointment.subject, appointment.message)} className="btn-facebook btn-brand icon mr-1 mb-1" title="View appointment"><i className="fa fa-eye"></i></Button>
                              </td>
                              <td>
                                <Button size="sm" onClick={() => this.toggleReply(appointment.id, appointment.name, appointment.email, appointment.subject, appointment.message)} className="btn-facebook btn-brand icon mr-1 mb-1" title="Reply appintment message"><i className="fa fa-reply"></i></Button>
                              </td>
                              <td>{this.state.status}</td>
                            </tr>
                          )
        ...
        <Modal isOpen={this.state.view} className={'modal-primary ' + this.props.className} style={{maxWidth: "1000px"}}>
          <ModalHeader toggle={() => this.toggleView("close", "close")}>View Appointment</ModalHeader>
          <ModalBody>
