...
var PortAccount = /*#__PURE__*/function (_Component) {
  ...
  function PortAccount(props) {
    ...
    _this.state = (_this$state = {
      ...
      currentPage2: '',
      status: '1'
    }, _defineProperty(_this$state, "status", ""), _defineProperty(_this$state, "created_at", localStorage["appState"] ? JSON.parse(localStorage["appState"]).user.created_at : ""), _defineProperty(_this$state, "user_type", localStorage["appState"] ? JSON.parse(localStorage["appState"]).user.user_type : ""), _defineProperty(_this$state, "fadeIn", true), _defineProperty(_this$state, "timeout", 300), _defineProperty(_this$state, "login_as", ""), _this$state);
  ...
  _createClass(PortAccount, [{
    ...
    value: function checkIfProfileCompleted() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_3___default.a.get("/api/port/get/" + this.state.id + "?token=".concat(this.state.token)).then(function (response) {
        return response;
      }).then(function (json) {
        if (json.data.success) {
